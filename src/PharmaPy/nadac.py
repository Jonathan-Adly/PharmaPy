...
def get_avg_from_ndc_description(drug_name, exact_match=False, starts_with=False):
    ...
    seen = []
    new_list = []
    latest_effective_date = results[0]["effective_date"]
    for item in results:
        if item["effective_date"] == latest_effective_date:
            if not item["ndc"] in seen:
                new_list.append(item)
                seen.append(item["ndc"])
   
    new_list = [list(v) for k, v in itertools.groupby(new_list, key=lambda k: k["ndc_description"])]
    ...
    for name_list in new_list:
        ...
        result.append({"ndc_description": description, "average_nadac_per_unit": str(average)})
    return result
